...
public class GeminiClient {
    ...
    /**
     * Generates an image using the Gemini Imagen endpoint.
     *
    ...
    public BufferedImage generateImage(String prompt) throws IOException {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            ...
            JsonObject json = gson.fromJson(responseBody, JsonObject.class);
            String base64Image = findBase64Image(json);
            ...
            return image != null ? image : buildFallbackImage(prompt);
    ...
    /**
     * Traverses common Imagen response shapes to locate a base64-encoded image.
     *
     * @param json the JSON response object
     * @return the base64 image string or null when not found
     */
    private String findBase64Image(JsonObject json) {
        if (json == null) {
            return null;
        }
        ...
        if (json.has("predictions")) {
            ...
            for (JsonElement element : predictions) {
                String image = extractImageField(element.getAsJsonObject());
                if (image != null) {
                    return image;
                }
            }
        ...
        return extractImageField(json);
    }

    /**
     * Extracts a known image field from a JSON object.
     *
     * @param object the JSON object to inspect
     * @return the base64 image string or null when absent
     */
    private String extractImageField(JsonObject object) {
        String[] keys = {"bytesBase64Encoded", "imageBytes", "b64Json", "image"};
        for (String key : keys) {
        ...
        if (object.has("inlineData")) {
            JsonObject inlineData = object.getAsJsonObject("inlineData");
            if (inlineData.has("data")) {
                return inlineData.get("data").getAsString();
            }
        }
